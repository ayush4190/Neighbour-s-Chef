 ...
 */
inline fun <reified T: Parcelable> Parcel.writeParcelableListCompat(`val`: List<T>?, flags: Int) {
    if (`val` == null) {
        writeInt(-1)
        return
    }
 ...
 */
fun Parcel.writeLocalDateTime(`val`: LocalDateTime) {
    writeLong(`val`.withNano(0).toEpochSecond(ZoneOffset.UTC))
}
 ...
 */
inline fun <reified T: Enum<T>> Parcel.writeEnum(`val`: T) = writeString(`val`.name)